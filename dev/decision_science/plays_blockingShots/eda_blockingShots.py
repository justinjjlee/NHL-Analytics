...
# %% EDA: Sequence for shot blocker team
dfblock.loc[(dfblock.typeDescKey == "shot-on-goal") & (dfblock.eventOwnerTeam_sequence == "Own action") & 
(dfblock.blocker_team_lag1_now == "Defense")]



dfblock.loc[1540:1550, :]



# %% Data specifically focused on blocked shots
dfblock = dbnhl.conn.execute(
    """
        SELECT 
            seasonIdx
...
tempdf = dfblock.groupby(['blocker_team_lag1_now', 'typeDescKey', 'eventOwnerTeam_sequence']).seasonIdx.count().reset_index()
table_pivot = tempdf.pivot(index='typeDescKey', columns=['eventOwnerTeam_sequence','blocker_team_lag1_now'], values='seasonIdx')

# HIgher-level grouping
dfmatch = pd.DataFrame({
    'typeDescKey':[
        'blocked-shot', 'faceoff', 'giveaway', 'goal', 'hit',
        'missed-shot', 'penalty', 'shot-on-goal', 'takeaway',
        'delayed-penalty', 'game-end', 'period-end', 'stoppage'],
    'group':[
        'Shot', 'Stoppage', 'Possession', 'Goal', 'Possession', 
        'Shot', 'Penalty', 'Shot', 'Possession',
        'Penalty', 'Stoppage', 'Stoppage', 'Stoppage'

    ]
})

tempdf = pd.merge(tempdf, dfmatch, on='typeDescKey', how='left')

tempdf.to_csv(str_cwd + '/result/table - sequence blocked shot stats.csv', index=False)
...
'''
The Human Shield Analysis

'details.eventOwnerTeam' of the "blocked-shot" is the team shooting
...
-- Given the blocked shots from the perivous period --
Success critera
p(zone exit | blocked shots)
p(blocked shots | blocked shots)
p(stoppage of play | blocked shots)
p(defensive takeaway | blocked shots)

easily filter by event team on those conditions (need to organize)


Failure criteria 
p(goal scored against | blocked shots)
p(giveaway by the same team | blocked shots)


-- Versus blocked shots
missed-shot
shot-on-goal

Need to account for power-play/penality kill events.
: systematically capture rolling activities within the time?

( check each row, there are non-null value, with another column with specific integer value)
(any subsequent rows within the time frame would be considered power play activities)
'''

# How the zone area would impact ?
# %% All blocked shots alike? PCA approach


# %% Utility function bellman curve approach
...
# Show the plot
plt.savefig(str_cwd + '/result/figure - eda - sequence of posture after blocking shot', dpi=600)